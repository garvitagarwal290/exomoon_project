...
for n in range(len(catalogue)-3):
    ...
    plt.scatter(timestamps, trend, s=0.5)
    plt.xlabel('Time (days)')
    plt.ylabel('Raw flux value')
    plt.show()

    final_lc = np.ndarray((rebinned_time.shape[0]))
    for j in range(rebinned_time.shape[0]):
        final_lc[j] = detrended_flux[np.where(timestamps == rebinned_time[j])]
    plt.scatter(rebinned_time, final_lc, s=0.5, color='black')
    plt.show()
    ...
    while(True):
        ...
        num = int(input("number of regions to exclude: \n"))
        for j in range(num):
        ...
        plt.scatter(timestamps_excl, detrended_flux_excl, s=0.5, color='black')
        plt.show()        
        ...
        plt.scatter(rebinned_time_excl, final_lc, s=0.5, color='black')
        plt.show()

        residual = injected_flux_excl - trend_excl
        plt.scatter(timestamps_excl, residual, s=0.5, color='black')
        plt.show()

        mask_idxs = mask_indices(timestamps_excl)
        masked_residual = np.delete(residual, mask_idxs)
        plt.scatter(np.arange(masked_residual.shape[0]), masked_residual, s=0.5, color='black')
        plt.show()

        done = input("Exclusion Done?\n")
        if(done == 'yes'): break
    ...
    print(dw_statistic, float(line[15]))
    
    
    print("Writing the injected light curve for this system in a file\n-----------------------------------------------------\n")
    f = open('/home/garvit/Downloads/Exomoon_project/final_lightcurves/final_lightcurve_{}_{}_{}.csv'.format(date, res_nores, sim_no), 'w')
